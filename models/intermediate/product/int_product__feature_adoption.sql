...
user_licenses as (
    select * from {{ ref('stg_product__user_licenses') }}
),
...
feature_usage as (
    select
        ...
        u.customer_id,
        u.feature_name,
        cast({% if target.type == 'bigquery' %}date_trunc(u.event_date, month){% else %}date_trunc('month', u.event_date){% endif %} as date) as usage_month,
